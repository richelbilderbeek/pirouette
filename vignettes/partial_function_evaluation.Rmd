...
I'll demonstrate using the `create_sim_bd_twin_tree_fun`
...
There is a helper function called `create_sim_bd_twin_tree_fun`
...
How to specify *how* the true alignment is created? Use 
the `alignment_params$sim_true_alignment_fun`:
...
Due to this architecture, `alignment_params$sim_true_alignment_fun` must
be a function that takes *one* argument, which is the true phylogeny.
...
=======
So, `alignment_params$sim_true_alignment_fun` must
>>>>>>> b31a67ccf7a115ac420237774dfccbe724a0a7fa
be a function that takes *one* argument, which is the true phylogeny. What
if we want a function that indeed has one argument for the true phylogeny,
but also some additional ones?
...
=======
An example is `sim_true_alignment_with_std_site_model`, which uses a 
root sequence of `acgt`, a mutation rate of 0.1 and a JC69 site model:

```{r}
head(sim_true_alignment_with_std_site_model)
>>>>>>> b31a67ccf7a115ac420237774dfccbe724a0a7fa
```
...
=======
sim_true_alignment_fun <- pryr::partial(
  sim_true_alignment_with_std_site_model, 
  mutation_rate = 0.5,                     
  site_model = beautier::create_hky_site_model()
)
head(sim_true_alignment_fun)
>>>>>>> b31a67ccf7a115ac420237774dfccbe724a0a7fa
```
...
=======
supplies this function as `get_sim_true_alignment_with_std_site_model_fun`:
...
```{r}
head(sim_twin_alignment_with_std_site_model)
```
...
check_sim_twin_alignment_fun(sim_twin_align_fun)
>>>>>>> b31a67ccf7a115ac420237774dfccbe724a0a7fa
```
...
<<<<<<< HEAD
  ...
  newskool = TRUE
=======
    sim_twin_alignment_fun = sim_twin_align_fun
  )
>>>>>>> b31a67ccf7a115ac420237774dfccbe724a0a7fa
)
ape::image.DNAbin(twin_alignment, main = "Twin alignment", legend = FALSE)
...
<<<<<<< HEAD
=======
Or use `get_sim_twin_alignment_with_std_site_model_fun` to do the 
partial function evaluation for you:
