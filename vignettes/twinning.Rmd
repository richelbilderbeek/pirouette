...
```{r}
testit::assert(file.exists(pir_params$experiments[[2]]$beast2_options$output_trees_filenames))
babette::plot_densitree(
  tracerer::parse_beast_trees(pir_params$experiments[[2]]$beast2_options$output_trees_filenames)
)
```
...
```{r}
testit::assert(file.exists(pir_params$experiments[[2]]$beast2_options$output_log_filename))
df <- tracerer::parse_beast_log(experiment_2$beast2_options$output_log_filename)
ggplot(data = df, aes(x = Sample, y = likelihood)) + geom_line()
```
