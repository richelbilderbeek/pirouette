...
if (1 == 2) {
  ...
  ape::read.dna(file = alignment_params$fasta_filename)
}
```
...
```{r}
testit::assert(file.exists(experiment_1$beast2_options$output_trees_filenames))
babette::plot_densitree(
  tracerer::parse_beast_trees(experiment_1$beast2_options$output_trees_filenames)
)
```
...
```{r}
testit::assert(file.exists(experiment_2$beast2_options$output_trees_filenames))
babette::plot_densitree(
  tracerer::parse_beast_trees(experiment_2$beast2_options$output_trees_filenames)
)
```
...
```{r}
testit::assert(file.exists(experiment_1$beast2_options$output_log_filename))
df <- tracerer::parse_beast_log(experiment_1$beast2_options$output_log_filename)
...
```{r}
testit::assert(file.exists(experiment_2$beast2_options$output_log_filename))
...
ape::image.DNAbin(ape::read.FASTA(file = twinning_params$twin_alignment_filename))
```
