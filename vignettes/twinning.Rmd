...
if (rappdirs::app_dir()$os != "win") {
  ...
  # Must share same BEAST2 options and errors filenae
...
```{r}
if (rappdirs::app_dir()$os != "win" && is_beast2_installed() && is_beast2_ns_pkg_installed()) {
...
```{r}
if (rappdirs::app_dir()$os != "win" && is_beast2_installed() && is_beast2_ns_pkg_installed()) {
  check_file_exists(pir_params$experiments[[1]]$beast2_options$output_trees_filenames)
...
```{r}
if (rappdirs::app_dir()$os != "win" && is_beast2_installed() && is_beast2_ns_pkg_installed()) {
  check_file_exists(pir_params$experiments[[1]]$inference_model$mcmc$tracelog$filename)
  df <- tracerer::parse_beast_log(pir_params$experiments[[1]]$inference_model$mcmc$tracelog$filename)
...
```{r}
if (rappdirs::app_dir()$os != "win" && is_beast2_installed() && is_beast2_ns_pkg_installed()) {
  log_filename <- pir_params$experiments[[2]]$inference_model$mcmc$tracelog$filename
...
```{r}
if (rappdirs::app_dir()$os != "win" && is_beast2_installed() && is_beast2_ns_pkg_installed()) {
  log_filename <- to_twin_filename(pir_params$experiments[[2]]$inference_model$mcmc$tracelog$filename)
  check_file_exists(log_filename)
  df <- tracerer::parse_beast_log(log_filename)
  ggplot(data = df, aes(x = Sample, y = likelihood)) + geom_line()
}
