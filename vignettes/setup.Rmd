...
`model_type`|`run_if`        |`measure_evidence`|`beautier::inference_model`
------------|----------------|------------------|---------------------------
`generative`|`always`        |`FALSE`           |`generative_model`

We assume we know the generative model, which
happens to be the BEAUti default:

```{r}
generative_model <- create_old_skool_inference_model()
...
```{r}
inference_model_1 <- create_old_skool_inference_model()
...
inference_model_2 <- create_old_skool_inference_model(
...
inference_model_3 <- create_old_skool_inference_model(
