...
test_that("use linked_node_sub, newskool", {
  phylogeny <- ape::read.tree(text = "(((A:1, B:1):1, C:2):1, D:3);")
  root_sequence <- "aaccggtt"
  alignment_params <- create_alignment_params(
    sim_true_alignment_function = get_sim_true_alignment_with_linked_node_sub_site_model_function(
...
test_that("use linked_node_sub, oldskool", {
  phylogeny <- ape::read.tree(text = "(((A:1, B:1):1, C:2):1, D:3);")
  alignment_params <- create_alignment_params(
...
test_that("use unlinked_node_sub, newskool", {
  root_sequence <- "aaaa"
  phylogeny <- ape::read.tree(text = "(((A:1, B:1):1, C:2):1, D:3);")
  alignment_params <- create_alignment_params(
...
test_that("use unlinked_node_sub, oldskool", {
  phylogeny <- ape::read.tree(text = "(((A:1, B:1):1, C:2):1, D:3);")
  alignment_params <- create_alignment_params(
