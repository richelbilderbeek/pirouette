  ...
  for (experiment in experiments) {
    ...
    for (i in seq(1, nrow(marg_liks))) {
      ...
      testthat::expect_true(
        marg_liks_names[1] %in% beautier::get_site_model_names()
      )
      testthat::expect_true(
        marg_liks_names[2] %in% beautier::get_clock_model_names()
      )
      testthat::expect_true(
        marg_liks_names[3] %in% beautier::get_tree_prior_names()
      )
      testthat::expect_true(
        experiment_names[1] %in% beautier::get_site_model_names()
      )
      testthat::expect_true(
        experiment_names[2] %in% beautier::get_clock_model_names()
      )
      testthat::expect_true(
        experiment_names[3] %in% beautier::get_tree_prior_names()
      )
