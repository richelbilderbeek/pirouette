...
#' @seealso
#' Use \link{check_sim_true_alignment_fun} to check a function to
#' generate a true alignment.
#' Use \link{check_sim_twin_tree_fun} to check a function to
#' generate a twin phylogeny.
#' @export
check_sim_twin_alignment_fun <- function(sim_twin_alignment_fun) {
  if (!is.function(sim_twin_alignment_fun)) {
    stop("'sim_twin_alignment_fun' must be a function")
  }

  # sim_twin_alignment_fun must return a DNAbin
  ...
  tryCatch({
      ...
      stop(
        "'sim_twin_alignment_fun' failed to run cleanly on test input. \n",
        "Error: ", e$message
      )
    }
  )
  ...
  tryCatch(
    ...
    condition = function(c) {
      stop(
        "'sim_twin_alignment_fun' must be a function that returns an ape::DNAbin" # nolint indeed a long string
      )
    }
  )
